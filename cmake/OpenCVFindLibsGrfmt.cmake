...
function( hex_digit_dec hex dec )
    if( ${hex} STREQUAL "a" )
        set( result 10 )
    ...
    set( ${dec} ${result} PARENT_SCOPE )
endfunction()

function( hex_to_dec hex dec )
    if( NOT hex )
        set( ${dec} 0 PARENT_SCOPE )
        return()
    endif()
    string( LENGTH ${hex} len )
    if( ${len} GREATER 2 )
        message( SEND_ERROR "hex_to_dec: Input hex string too long. ${hex}, length ${len}" )
        return()
    endif()

    string( TOLOWER ${hex} lower_hex )
    set( expr 0 )
    while( lower_hex )
        string( SUBSTRING ${lower_hex} 0 1 char )
        hex_digit_dec( ${char} dec_char )
        set( expr "${expr}*16+${dec_char}" )
        string( SUBSTRING ${lower_hex} 1 -1 lower_hex )
    endwhile()
    math( EXPR result ${expr} )
    set( ${dec} ${result} PARENT_SCOPE )
endfunction()

if(NOT ${WEBP_INCLUDE_DIR} STREQUAL "")
  ...
  set(FILENAME "${WEBP_INCLUDE_DIR}/webp/encode.h")
  if(EXISTS "${FILENAME}")
    file(STRINGS "${FILENAME}" WEBP_ENCODER_ABI_VERSION REGEX "#define[ \t]+(WEBP_ENCODER_ABI_VERSION)[ \t]+[x0-9]+" )
    string(REGEX REPLACE ".+[ \t]WEBP_ENCODER_ABI_VERSION[ \t]+([x0-9]+).*" "\\1" WEBP_ENCODER_ABI_VERSION ${WEBP_ENCODER_ABI_VERSION})
    set (ENC_MAJ_VERSION "0")

    string(SUBSTRING "${WEBP_ENCODER_ABI_VERSION}" 2 2 HEX_MIN)
    string(SUBSTRING "${WEBP_ENCODER_ABI_VERSION}" 4 2 HEX_REV)

    hex_to_dec(${HEX_MIN} ENC_MIN_VERSION)
    hex_to_dec(${HEX_REV} ENC_REV_VERSION)

    if (${WEBP_VERSION} STREQUAL "")
      set(WEBP_VERSION "${ENC_MAJ_VERSION}.${ENC_MIN_VERSION}.${ENC_REV_VERSION}")
    endif()
  endif()
endif()
